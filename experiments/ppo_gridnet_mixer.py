...
# implementation of MLP-Mixer is copied from:
...
class MixerActor(nn.Module):

    def __init__(self, minimap_dim=64, map_size=(16,16), input_channels=27, output_channels=78):
        super().__init__()
        self.map_h, self.map_w = map_size
        self.network = nn.Sequential(
            nn.Linear(input_channels+minimap_dim, 64),
            nn.ReLU(),
            nn.Linear(64, output_channels),
        )

    def forward(self, x, minimap):
        B = x.size(0)
        x = x.reshape((B, self.map_h*self.map_w, -1))
        minimap = minimap.unsqueeze(1).repeat((1, self.map_h*self.map_w, 1))
        logits = self.network(torch.cat([minimap, x], dim=-1))
        logits = logits.reshape((B, self.map_h, self.map_w, -1))
        return logits
...
class Agent(nn.Module):
    def __init__(self, envs, mapsize=16 * 16):
        ...
        self.encoder = nn.Sequential(
            Transpose((0, 3, 1, 2)),
            MLPMixer(image_size=(16,16), channels=c, patch_size=2, dim=32, depth=2, num_classes=64),
        )
        ...
        self.actor = MixerActor(minimap_dim=64, input_channels=27, output_channels=78)
